...
scpi_estimates_file <- "results/FigB1/scpi_estimates_controls.csv"
scpi_inference_file <- "results/FigB1/scpi_inference_controls.csv"
scpi_weights_file <- "results/FigB1/scpi_weights_controls.csv"
...
if (!file.exists(scpi_estimates_file) | !file.exists(scpi_inference_file) | !file.exists(scpi_weights_file)) {
  ...
  gdpi_res <- feols(gdpi ~ importss + exportss + log(ex_rate) + log(int_rate), foo)  # Estimate residuals from log GDP on exchange rate and interest rate
...
scpi_estimates <- read_csv("results/FigB1/scpi_estimates_controls.csv")
scpi_inference <- read_csv("results/FigB1/scpi_inference_controls.csv")
...
ggsave("results/FigB1/FigB1.png", dpi = 300, width = 7.4, height = 4.8)
