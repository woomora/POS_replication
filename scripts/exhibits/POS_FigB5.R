...
scpi_estimates_file <- "results/FigB5/scpi_estimates_controls.csv"
scpi_inference_file <- "results/FigB5/scpi_inference_controls.csv"
scpi_weights_file <- "results/FigB5/scpi_weights_controls.csv"
...
if (!file.exists(scpi_estimates_file) | !file.exists(scpi_inference_file) | !file.exists(scpi_weights_file)) {
  ...
  foo <- posq |> 
    filter(countrycode %in% c("ARG", "BRA", "CHL", "COL", "CRI", "HND", "ECU", "MEX", "CAN")) |> 
    filter(year >= 2013) |> 
...
scpi_estimates <- read_csv("results/FigB5/scpi_estimates_controls.csv")
scpi_inference <- read_csv("results/FigB5/scpi_inference_controls.csv")
...
ggsave("results/FigB5/FigB5.png", dpi = 300, width = 7.4, height = 4.8)
